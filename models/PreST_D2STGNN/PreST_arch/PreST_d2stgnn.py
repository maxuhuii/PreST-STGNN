...
class PreST_d2stgnn(nn.Module):
    ...
    def __init__(self, dataset_name, pre_trained_tsformer_path, pre_trained_stformer_path, tsformer_args, stformer_args, backend_args, dgl_args):
        ...
        self.backend = D2STGNN(**backend_args)
    ...
    def forward(self, history_data: torch.Tensor, long_history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, **kwargs) -> torch.Tensor:
        ...
        y_hat = self.backend(history_data=back_end_input, future_data=future_data, hidden_states=hidden_states,hidden_states_st=hidden_states_st, sampled_adj=sampled_adj,batch_seen=batch_seen)
        ...
        if epoch is not None:
            ...
            gsl_coefficient = 0
        return y_hat, bernoulli_unnorm.softmax(-1)[..., 0].clone().reshape(batch_size, num_nodes, num_nodes), adj_knn, gsl_coefficient
