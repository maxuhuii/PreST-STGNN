...
class STLocalizedConv(nn.Module):
    ...
    def get_graph2(self, support):
        # Only used in static including static hidden graph and predefined graph, but not used for dynamic graph.
        # 输入由NXN->BXNXN
        graph_ordered = []
        mask = 1 - torch.eye(support[0].shape[1]).to(support[0].device)
        mask = mask.expand(support[0].shape[0],-1,-1)
        for graph in support:
            k_1_order = graph                           # 1 order
            graph_ordered.append(k_1_order * mask)
            # e.g., order = 3, k=[2, 3]; order = 2, k=[2]
        ...
        for graph in graph_ordered:
            # B,N,N->B,N,KT,N
            graph = graph.unsqueeze(-2).expand(-1, -1, self.k_t, -1)
    ...
    def forward(self, X, dynamic_graph, static_graph, learn_graph):
        # X: [bs, seq, nodes, feat]
        ...
        if self.use_predefined_graph:
            self.pre_defined_graph = self.get_graph2(learn_graph)
