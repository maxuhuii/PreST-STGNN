...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, num_nodes, supports, dropout=0.3, gcn_bool=True, addaptadj=True, aptinit=None, in_dim=2,out_dim=12,residual_channels=32,dilation_channels=32,skip_channels=256,end_channels=512,kernel_size=2,blocks=4,layers=2, **kwargs):
        ...
        self.start_conv_st = nn.Conv2d(in_channels=96, out_channels=residual_channels, kernel_size=(1,1))
    ...
    def forward(self, input, hidden_states, hidden_states_st, sampled_adj):
        ...
        # reshape input: [B, L, N, C] -> [B, C, N, L]
         # x:[32,32,207,13]  hidden_states_st:[32,207,12,96]->[32,96,207,12]
        input = input.transpose(1, 3)
        hidden_states_st = hidden_states_st.permute(0,3,1,2)
        # feed forward
        input = nn.functional.pad(input,(1,0,0,0))
        hidden_states_st = nn.functional.pad(hidden_states_st,(1,0,0,0))
        ...
        in_len_st = hidden_states_st.size(3)
        if in_len_st<self.receptive_field:
            x_xt = nn.functional.pad(hidden_states_st,(self.receptive_field-in_len_st,0,0,0))
        else:
            x_xt = hidden_states_st

        x = self.start_conv(x)
        x_xt = self.start_conv_st(x_xt)

        x = self.relu(x+x_xt)
        # x:[32,32,207,13]  hidden_states_st:[32,207,12,96]
        ...
        # skip:[32,256,207,1] hidden_states:[32,256,207,1] hidden_states_st:32,207,12,96
